...
//Definir función ANN
def ANN(series:Array[Double],alpha:Double,h:Int,ept:Double)= { 
...
{	
	...
	if (n < series.length)
	{
		...
		et += series(n)-lastlevel.last
		level += (1-alpha)*lastlevel.last+alpha*series(n)
		result += lastlevel.last
	}
	if (n >= series.length)
	{
		lastlevel += level.last
		et += ept
		level += lastlevel.last + et.last * alpha
		result += lastlevel.last 
	}	
...
for (a<-1 to 3)
{
		...
		{	
			...
			{
				val result     = ANN((vector.slice(0,i)),alpha,1,0)
				val forecast   = ANN((vector.slice(0,vector.length)),alpha,1,0)
...
val fit = ANN(vector,bestalpha.last,16,et.last)
...
val fit = ANN(vector,bestalpha.last,16,0)
//Recuperar datos ajustados al modelo a escala inicial
val VectorPredict = new ArrayBuffer[Double]()
