...
//Definir función AAA
def AAA(series:Array[Double],alpha:Double,beta:Double,gamma:Double,h:Int,ept:Double)= { 
...
{	
	...
	if (n < series.length)
	{
		...
		trend += lasttrend.last +  beta*et.last
...
for (a<-1 to 3)
{
	for (b<-1 to 100)
	{
			...
			if( alpha>beta && gamma<1-alpha)
			{
				...
				{
					val result     = AAA((vector.slice(0,i)),alpha,beta,gamma,1,0)
					val forecast   = AAA((vector.slice(0,vector.length)),alpha,beta,gamma,1,0)
...
val fit = AAA(vector,bestalpha.last,bestbeta.last,bestgamma.last,16,et.last)
...
val fit = AAA(vector,bestalpha.last,bestbeta.last,bestgamma.last,16,0)
//Recuperar datos ajustados al modelo a escala inicial
val VectorPredict = new ArrayBuffer[Double]()
