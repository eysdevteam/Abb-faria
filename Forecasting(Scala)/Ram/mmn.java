...
//Definir función MMN
def MMN(series:Array[Double],alpha:Double,beta:Double,h:Int,ept:Double)= { 
...
{	
	...
	if (n < series.length)
	{
		...
		level += (1-alpha) * lastlevel.last*lasttrend.last + alpha*series(n)
		trend += (1-beta/alpha)*lasttrend.last + beta/alpha*level.last/lastlevel.last
	...
	if (n >= series.length)
	{
		...
		lasttrend += trend.last
		et += ept
		level += lastlevel.last * lasttrend.last *  (1+alpha*et.last)
		trend += lasttrend.last * (1+ beta*et.last)
		result += lastlevel.last * lasttrend.last
	}	
...
for (a<-1 to 3)
{
	for(b <- 1 to 100)
	{
		...
		if(alpha > beta)
		{
			...
			{
				val result     = MMN((vector.slice(0,i)),alpha,beta,1,0)
				val forecast   = MMN((vector.slice(0,vector.length)),alpha,beta,1,0)
...
val fit = MMN(vector,bestalpha.last,bestbeta.last,16,et.last)
...
val fit = MMN(vector,bestalpha.last,bestbeta.last,16,0)
//Recuperar datos ajustados al modelo a escala inicial
val VectorPredict = new ArrayBuffer[Double]()
