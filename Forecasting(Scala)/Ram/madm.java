...
//Definir función MAdM
def MAdM(series:Array[Double],alpha:Double,beta:Double,gamma:Double,phi:Double,h:Int,ept:Double)= { 
...
for (a<-1 to 3)
{
	for (b<-1 to 100)
	{
			...
			if( alpha>beta && gamma<1-alpha)
			{
				...
				{
					val result     = MAdM((vector.slice(0,i)),alpha,beta,gamma,phi,1,0)
					val forecast   = MAdM((vector.slice(0,vector.length)),alpha,beta,gamma,phi,1,0)
...
val fit = MAdM(vector,bestalpha.last,bestbeta.last,bestgamma.last,bestphi.last,16,et.last)
...
val fit = MAdM(vector,bestalpha.last,bestbeta.last,bestgamma.last,bestphi.last,16,0)
//Recuperar datos ajustados al modelo a escala inicial
val VectorPredict = new ArrayBuffer[Double]()
