...
//Definir función MAdN
def MAdN(series:Array[Double],alpha:Double,beta:Double,phi:Double,h:Int,ept:Double)= { 
...
{	
	...
	if (n < series.length)
	{
		...
		level += (lastlevel.last + phi*lasttrend.last) *  (1+alpha*et.last)
		trend += phi * lasttrend.last +  beta*(lastlevel.last+phi*lasttrend.last)*et.last
		result += lastlevel.last + phi*lasttrend.last
	}
	if (n >= series.length)
	{
		lastlevel += level.last
...
for (a<-1 to 3)
{
	for(b <- 1 to 100)
	{
		...
		if(alpha>beta)
		{
			...
			{
				val result     = MAdN((vector.slice(0,i)),alpha,beta,phi,1,0)
				val forecast   = MAdN((vector.slice(0,vector.length)),alpha,beta,phi,1,0)
...
val fit = MAdN(vector,bestalpha.last,bestbeta.last,bestphi.last,16,et.last)
...
val fit = MAdN(vector,bestalpha.last,bestbeta.last,bestphi.last,16,0)
//Recuperar datos ajustados al modelo a escala inicial
val VectorPredict = new ArrayBuffer[Double]()
