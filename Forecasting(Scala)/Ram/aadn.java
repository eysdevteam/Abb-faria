...
//Definir función AAdN
def AAdN(series:Array[Double],alpha:Double,beta:Double,phi:Double,h:Int,ept:Double)= { 
...
{	
	...
	if (n < series.length)
	{
		...
		et += series(n)-lastlevel.last-lasttrend.last
		level += lastlevel.last + phi * lasttrend.last + alpha * et.last
		trend += phi* lasttrend.last + beta*alpha*et.last
...
for (a<-1 to 3)
{
	for(b <- 1 to 100)
	{
		...
		if(alpha> beta)
		{
			...
			{
				val result     = AAdN((vector.slice(0,i)),alpha,beta,phi,1,0)
				val forecast   = AAdN((vector.slice(0,vector.length)),alpha,beta,phi,1,0)
...
val fit = AAdN(vector,bestalpha.last,bestbeta.last,bestphi.last,16,et.last)
...
val fit = AAdN(vector,bestalpha.last,bestbeta.last,bestphi.last,16,0)
//Recuperar datos ajustados al modelo a escala inicial
val VectorPredict = new ArrayBuffer[Double]()
...
println("bo : " +      math.log(lrModel.coefficients(0))/math.log(lrModel.intercept))
