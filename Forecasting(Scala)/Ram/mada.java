...
val rdd = sc.textFile("/home/ed/spark-1.6.0-bin-hadoop2.6/examples/src/main/resources/opentables.txt")
...
//Definir función MAdA
def MAdA(series:Array[Double],alpha:Double,beta:Double,gamma:Double,phi:Double,h:Int,ept:Double)= { 
...
{	
	...
	if (n < series.length)
	{
		...
		et += (series(n)- (lastlevel.last + phi*lasttrend.last + lastseasonal.last))/ (lastlevel.last + phi*lasttrend.last + lastseasonal.last)
		level += lastlevel.last + phi*lasttrend.last +  alpha*(lastlevel.last + phi*lasttrend.last+lastseasonal.last)*et.last
		trend += phi*lasttrend.last +  beta*(lastlevel.last + phi*lasttrend.last+lastseasonal.last)*et.last
		seasonal += lastseasonal.last + gamma*(lastlevel.last + phi*lasttrend.last+lastseasonal.last)*et.last
	...
	if (n >= series.length)
	{
		...
		et += ept
		level += lastlevel.last + lasttrend.last +  alpha*(lastlevel.last+lasttrend.last+lastseasonal.last)*et.last
		trend += lasttrend.last +  beta*(lastlevel.last+lasttrend.last+lastseasonal.last)*et.last
		seasonal += lastseasonal.last + gamma*(lastlevel.last+lasttrend.last+lastseasonal.last)*et.last
		result   += lastlevel.last + lasttrend.last + lastseasonal.last
	}	
	...
	for (b<-1 to 100)
	{
		...
		val phi = (80 + r.nextInt(( 98 - 80) + 1)).toDouble/100
			...
			for(i <- 3 until (vector.length) )
				{
					val result     = MAdA((vector.slice(0,i)),alpha,beta,gamma,phi,1,0)
					val forecast   = MAdA((vector.slice(0,vector.length)),alpha,beta,gamma,phi,1,0)
				...
				if(lik.last < bestlik.last)
				{
				bestlik    += lik.last
				bestloglik += -0.4918547*lik.last
...
val fit = MAdA(vector,bestalpha.last,bestbeta.last,bestgamma.last,bestphi.last,16,et.last)
...
val fit = MAdA(vector,bestalpha.last,bestbeta.last,bestgamma.last,bestphi.last,16,0)
//Recuperar datos ajustados al modelo a escala inicial
val VectorPredict = new ArrayBuffer[Double]()
