...
//Definir función MAM
def MAM(series:Array[Double],alpha:Double,beta:Double,gamma:Double,h:Int,ept:Double)= { 
...
for (a<-1 to 3)
{
	for (b<-1 to 100)
	{
			...
			if( alpha>beta && gamma<1-alpha)
			{
				...
				{
					val result     = MAM((vector.slice(0,i)),alpha,beta,gamma,1,0)
					val forecast   = MAM((vector.slice(0,vector.length)),alpha,beta,gamma,1,0)
...
val fit = MAM(vector,bestalpha.last,bestbeta.last,bestgamma.last,16,et.last)
...
val fit = MAM(vector,bestalpha.last,bestbeta.last,bestgamma.last,16,0)
//Recuperar datos ajustados al modelo a escala inicial
val VectorPredict = new ArrayBuffer[Double]()
