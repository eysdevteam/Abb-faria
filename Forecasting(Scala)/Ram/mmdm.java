...
//Definir función MMdM
def MMdM(series:Array[Double],alpha:Double,beta:Double,gamma:Double,phi:Double,h:Int,ept:Double)= { 
	...
	trend += 1+ (((vector.slice(4,8).sum/4) - level.last)/4)
...
{	
	...
	if (n < series.length)
	{
		...
		et += (series(n)- (lastlevel.last * math.pow(lasttrend.last,phi) * lastseasonal.last)) / (lastlevel.last * math.pow(lasttrend.last,phi) * lastseasonal.last)
		level    += lastlevel.last * math.pow(lasttrend.last,phi) *  (1 + alpha*et.last)
		trend    += math.pow(lasttrend.last,phi) * (1+beta*et.last)
		seasonal += lastseasonal.last *( 1 + gamma*et.last)
		result   += (lastlevel.last * math.pow(lasttrend.last,phi) * lastseasonal.last)
	}
	if (n >= series.length)
	{
		lastlevel += level.last
...
for (a<-1 to 1)
{
	for (b<-1 to 100)
	{
			...
			if( alpha>beta && gamma<1-alpha)
			{
				...
				{
					val result     = MMdM((vector.slice(0,i)),alpha,beta,gamma,phi,1,0)
					val forecast   = MMdM((vector.slice(0,vector.length)),alpha,beta,gamma,phi,1,0)
...
val fit = MMdM(vector,bestalpha.last,bestbeta.last,bestgamma.last,bestphi.last,16,et.last)
...
val fit = MMdM(vector,bestalpha.last,bestbeta.last,bestgamma.last,bestphi.last,16,0)
//Recuperar datos ajustados al modelo a escala inicial
val VectorPredict = new ArrayBuffer[Double]()
