...
//Definir función MMdN
def MMdN(series:Array[Double],alpha:Double,beta:Double,phi:Double,h:Int,ept:Double)= { 
    ...
    level += math.log(lrModel.intercept)
    trend += 1 +  (math.log(lrModel.coefficients(0))/math.log(lrModel.intercept))
    //level += 11.9707
    //trend += 1.0009
...
{	
	...
	if (n < series.length)
	{
		...
		et += (series(n)-(lastlevel.last * math.pow(lasttrend.last,phi)))/(lastlevel.last * math.pow(lasttrend.last,phi))
		level += lastlevel.last * math.pow(lasttrend.last,phi) *  (1+alpha*et.last)
		trend += math.pow(lasttrend.last,phi) * (1+ beta*et.last)
		result += (lastlevel.last * math.pow(lasttrend.last,phi))
	}
	if (n >= series.length)
	{
		lastlevel += level.last
...
for (a<-1 to 3)
{
	for(b <- 1 to 100)
	{
		...
		if(alpha>beta)
		{
			...
			{
				val result     = MMdN((vector.slice(0,i)),alpha,beta,phi,1,0)
				val forecast   = MMdN((vector.slice(0,vector.length)),alpha,beta,phi,1,0)
...
val fit = MMdN(vector,bestalpha.last,bestbeta.last,bestphi.last,16,et.last)
...
val fit = MMdN(vector,bestalpha.last,bestbeta.last,bestphi.last,16,0)
//Recuperar datos ajustados al modelo a escala inicial
val VectorPredict = new ArrayBuffer[Double]()
...
println("lo : " + math.log(lrModel.intercept))
println("bo : " +      (1 + (math.log(lrModel.coefficients(0))/math.log(lrModel.intercept))))
