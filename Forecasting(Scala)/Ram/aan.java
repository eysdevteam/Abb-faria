...
//Definir función AAN
def AAN(series:Array[Double],alpha:Double,beta:Double,h:Int,ept:Double)= { 
...
{	
	...
	if (n < series.length)
	{
		...
		level  += lastlevel.last + lasttrend.last +  alpha*et.last
		trend  += lasttrend.last +  beta*alpha*et.last
...
for (a<-1 to 3)
{
	for(b <- 1 to 100)
	{
		...
		if(alpha>beta)
		{
			...
			{
				val result     = AAN((vector.slice(0,i)),alpha,beta,1,0)
				val forecast   = AAN((vector.slice(0,vector.length)),alpha,beta,1,0)
...
val fit = AAN(vector,bestalpha.last,bestbeta.last,16,et.last)
...
val fit = AAN(vector,bestalpha.last,bestbeta.last,16,0)
//Recuperar datos ajustados al modelo a escala inicial
val VectorPredict = new ArrayBuffer[Double]()
