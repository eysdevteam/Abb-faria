...
//Definir función MAN
def MAN(series:Array[Double],alpha:Double,beta:Double,h:Int,ept:Double)= { 
    ...
    trend += (math.log(lrModel.coefficients(0)))
    //level += 11.7742
    //trend += 0.0058
...
{	
	...
	if (n < series.length)
	{
		...
		et += (series(n)-lastlevel.last-lasttrend.last)/(lastlevel.last + lasttrend.last)
		level += alpha*series(n) + (1-alpha)*(lastlevel.last+lasttrend.last)
		trend += beta/alpha*(level.last-lastlevel.last)+(1-beta/alpha)*lasttrend.last
		result += lastlevel.last + lasttrend.last
	}
	if (n >= series.length)
	{
		lastlevel += level.last
		lasttrend += trend.last
		et += ept
		level += (lastlevel.last + lasttrend.last) *  (1+et.last*alpha)
		trend += lasttrend.last +  beta*(lastlevel.last+lasttrend.last)*et.last
		result += lastlevel.last + lasttrend.last
	}	
...
for (a<-1 to 3)
{
	for(b <- 1 to 100)
	{
		...
		if(alpha > beta)
		{
			...
			{
				val result     = MAN((vector.slice(0,i)),alpha,beta,1,0)
				val forecast   = MAN((vector.slice(0,vector.length)),alpha,beta,1,0)
...
val fit = MAN(vector,bestalpha.last,bestbeta.last,16,et.last)
...
val fit = MAN(vector,bestalpha.last,bestbeta.last,16,0)
//Recuperar datos ajustados al modelo a escala inicial
val VectorPredict = new ArrayBuffer[Double]()
