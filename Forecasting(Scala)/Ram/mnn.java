...
//Definir función MNN
def MNN(series:Array[Double],alpha:Double,h:Int,ept:Double)= { 
...
{	
	...
	if (n < series.length)
	{
		...
		et += (series(n)-lastlevel.last)/(lastlevel.last)
		level += lastlevel.last *  (1+et.last*alpha)
...
for (a<-1 to 3)
{
	for(b <- 1 to 100)
	{
		...
		{
			val result     = MNN((vector.slice(0,i)),alpha,1,0)
			val forecast   = MNN((vector.slice(0,vector.length)),alpha,1,0)
...
val fit = MNN(vector,bestalpha.last,16,et.last)
...
val fit = MNN(vector,bestalpha.last,16,0)
//Recuperar datos ajustados al modelo a escala inicial
val VectorPredict = new ArrayBuffer[Double]()
