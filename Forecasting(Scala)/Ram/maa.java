...
//Definir función MAA
def MAA(series:Array[Double],alpha:Double,beta:Double,gamma:Double,h:Int,ept:Double)= { 
...
{	
	...
	if (n < series.length)
	{
		...
		et += (series(n)-lastlevel.last-lasttrend.last-lastseasonal.last)/(lastlevel.last+lasttrend.last+lastseasonal.last)
...
for (a<-1 to 3)
{
	for (b<-1 to 100)
	{
			...
			if( alpha>beta && gamma<1-alpha)
			{
				...
				{
					val result     = MAA((vector.slice(0,i)),alpha,beta,gamma,1,0)
					val forecast   = MAA((vector.slice(0,vector.length)),alpha,beta,gamma,1,0)
...
val fit = MAA(vector,bestalpha.last,bestbeta.last,bestgamma.last,16,et.last)
...
val fit = MAA(vector,bestalpha.last,bestbeta.last,bestgamma.last,16,0)
//Recuperar datos ajustados al modelo a escala inicial
val VectorPredict = new ArrayBuffer[Double]()
