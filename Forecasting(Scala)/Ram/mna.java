...
//Definir función AAA
def MNA(series:Array[Double],alpha:Double,gamma:Double,h:Int,ept:Double)= { 
...
{	
	...
	if (n < series.length)
	{
		...
		et += (series(n)-lastlevel.last-lastseasonal.last) / (lastlevel.last+lastseasonal.last)
		level += lastlevel.last  +  alpha*(lastlevel.last+lastseasonal.last)*et.last
		seasonal += lastseasonal.last + gamma*(lastlevel.last+lastseasonal.last)*et.last
		result += lastlevel.last +  lastseasonal.last
	}
	if (n >= series.length)
	{
		lastlevel += level.last
...
for (a<-1 to 3)
{
	for (b<-1 to 100)
	{
		...
		if( gamma<1-alpha)
		{
			...
			{
				val result     = MNA((vector.slice(0,i)),alpha,gamma,1,0)
				val forecast   = MNA((vector.slice(0,vector.length)),alpha,gamma,1,0)
...
val fit = MNA(vector,bestalpha.last,bestgamma.last,16,et.last)
...
val fit = MNA(vector,bestalpha.last,bestgamma.last,16,0)
//Recuperar datos ajustados al modelo a escala inicial
val VectorPredict = new ArrayBuffer[Double]()
