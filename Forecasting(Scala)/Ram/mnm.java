...
//Definir función AAA
def MNM(series:Array[Double],alpha:Double,gamma:Double,h:Int,ept:Double)= { 
...
{	
	...
	if (n < series.length)
	{
		...
		level += lastlevel.last  * (1 + alpha*et.last)
...
for (a<-1 to 3)
{
	for (b<-1 to 100)
	{
		...
		if( gamma<1-alpha)
		{
			...
			{
				val result     = MNM((vector.slice(0,i)),alpha,gamma,1,0)
				val forecast   = MNM((vector.slice(0,vector.length)),alpha,gamma,1,0)
...
val fit = MNM(vector,bestalpha.last,bestgamma.last,16,et.last)
...
val fit = MNM(vector,bestalpha.last,bestgamma.last,16,0)
//Recuperar datos ajustados al modelo a escala inicial
val VectorPredict = new ArrayBuffer[Double]()
